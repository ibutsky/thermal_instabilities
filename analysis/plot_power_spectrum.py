...
def plot_power_spectrum(sim, tctf_list, beta_list, cr_list, diff_list = None, output = 50, field = 'drho'):
    color_list  = palettable.scientific.sequential.Batlow_6.mpl_colors
    fig, ax = plt.subplots(figsize = (6, 6))
    for tctf, beta, cr, diff, color in zip(tctf_list, beta_list, cr_list, diff_list, color_list):
        # load the simulation
        sim_loc = pt.get_sim_location(sim, tctf, beta, cr, diff = diff)
        ds_path = '%s/DD%04d/DD%04d'%(sim_loc, output, output)
        if not os.path.isfile(ds_path):
            continue
        ds = yt.load(ds_path)
        
        label = pt.get_label_name(compare, tctf, beta, cr)
    ...
    figname = pt.get_fig_name('power_spectrum', sim, compare, tctf_list[0], beta_list[0], \
                                            cr_list[0], diff_list[0], time = output, use_tctf = 1)
    print(figname)
