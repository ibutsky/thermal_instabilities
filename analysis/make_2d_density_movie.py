...
def plot_density_slices(ds, folder = '.'):
    ...
    s = yt.SlicePlot(ds, 'z', [('gas', 'density'), ('gas', 'pressure')])
    frb_s = s.frb
    p = yt.ProjectionPlot(ds, 'z', [('gas', 'density'), ('gas', 'velocity_y')], weight_field = 'ones')
    p.set_unit(('gas', 'velocity_y'), 'km/s')
    frb_p = p.frb

    ad = ds.all_data()
    ph = yt.PhasePlot(ad, ('gas', 'pressure'), ('gas', 'entropy'), ('gas', 'cell_mass'), weight_field = None, x_bins = 128, y_bins = 128)
    ph2 = yt.PhasePlot(ad, ('gas', 'y'), ('gas', 'tcool_tff_ratio'), ('gas', 'cell_mass'), weight_field = None, x_bins = 128, y_bins = 128)
    ph2.set_log(('gas', 'y'), False)
    ...
    for i, frb in enumerate([frb_s, frb_p]):
        print(i)
        xbins = frb['x'].in_units('kpc')
        ybins = frb['y'].in_units('kpc')
        rho   = frb['density']
    ...
    # plot velocity
    pcm = ax[1][2].pcolormesh(xbins, ybins, frb_p[('gas', 'velocity_y')], norm=SymLogNorm(1), \
...
make_movie_plots(output_list[0])
